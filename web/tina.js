// 统一发送消息的包装函数，根据Android和iOS进行不同的适配，同时支持浏览器上的调试
var WTYJSBridge = function() {
	this.init();
};

WTYJSBridge.prototype = {
	loginType : '',// 用户的token信息，可以以此作为验证凭证
	init : function() {
		var _this = this;
		_this.invoke('userInfo', {}, function(response) {
			_this.loginType = response.loginType;
		});
	},
	invoke : function(cmd, message, responseCallback) {
		try {
			var info = JSON.stringify(message);
			var w = info.indexOf('{');
			var r = info.indexOf(':');
			var result = "";
			var cmdInfo = "{" + "\"command\":\"" + cmd + "\"";
			if (w == -1 || r == -1) {
				result = cmdInfo + "}";
			} else {
				result = cmdInfo + "," + info.substring(w + 1);
			}
			// console.log('message = ' + result);
			if (window.AndroidWebViewJavascriptBridge) {
				ret_val = prompt(result);
				if (responseCallback) {
					// {"name": "CodePlayer"} // name属性的值是字符串，必须用双引号(不能用单引号)
					responseCallback(JSON.parse(ret_val));
				}
				return true;
			} else {
				// windows的方便调试的方式
...
// 由Android APP触发，使得JS知道外部是APP
function android_app_ready() {
	var readyEvent = document.createEvent('Events')
	readyEvent.initEvent('AndroidWebViewJavascriptBridge')
	window.AndroidWebViewJavascriptBridge = true;
...
if (!bridge) {
	bridge = new WTYJSBridge();
}

// 重新设置loginType
function refreshUserInfo(info) {
	var ret = JSON.parse(info);
	bridge.loginType = ret.loginType;
}
